...
if sel_sev:   df = df[df["Severity"].isin(sel_sev)]
...
n_cust  = len(df[df["Cust. Impact"]=="Yes"])
...
kpi(k7, "Cust. Impact",   n_cust, "#7C3AED", "affected")
...
with c2:
    ...
    fig2 = go.Figure(go.Pie(
        ...
        hole=0.55, marker_colors=[SEV_COLOR.get(s,"#ccc") for s in sev_counts.index],
        textinfo="label+value", hoverinfo="label+percent"))
    fig2.update_layout(title="Severity Split", showlegend=False,
...
with sort_col:
    sort_by = st.selectbox("Sort by", ["ID","Days Open","Severity","Status","Category"],
...
if sort_by == "Severity":
    ...
    df_show["_rank"] = df_show["Severity"].map(sev_rank)
...
for _, row in df_show.iterrows():
    ...
    with st.expander(
        ...
        with col_a:
            ...
            st.markdown(f"**Severity:** {sev_badge(row['Severity'])}", unsafe_allow_html=True)
        ...
        with col_b:
            ...
            st.markdown(f"**Cust. Impact:** {row.get('Cust. Impact','—')}")
        with col_c:
